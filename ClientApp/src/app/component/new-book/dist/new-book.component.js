"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
exports.__esModule = true;
exports.NewBookComponent = void 0;
var core_1 = require("@angular/core");
var forms_1 = require("@angular/forms");
var NewBookComponent = /** @class */ (function () {
    function NewBookComponent(service, fb, router) {
        this.service = service;
        this.fb = fb;
        this.router = router;
    }
    NewBookComponent.prototype.ngOnInit = function () {
        this.addBookForm = this.fb.group({
            id: [Math.floor(Math.random() * 1000)],
            title: [null, forms_1.Validators.required],
            author: [null, forms_1.Validators.required],
            description: [null, forms_1.Validators.required, forms_1.Validators.minLength(5), forms_1.Validators.maxLength(250)],
            rate: [null],
            dateStart: [null],
            dateEnd: [null]
        });
    };
    ;
    NewBookComponent.prototype.onSubmit = function () {
        var _this = this;
        this.service.addBook(this.addBookForm.value)
            .subscribe(function (b) {
            _this.router.navigate(["/books"]);
        });
    };
    NewBookComponent = __decorate([
        core_1.Component({
            selector: 'app-new-book',
            templateUrl: './new-book.component.html',
            styleUrls: ['./new-book.component.css']
        })
    ], NewBookComponent);
    return NewBookComponent;
}());
exports.NewBookComponent = NewBookComponent;
